...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Determine if we're on a video page
      const isVideoPage = pathname?.includes("/shows/")

      if (isVideoPage) {
        // On video pages, hide navbar when scrolling down, show when scrolling up
        ...
        // On other pages, just add background when scrolled
        setHideNavbar(false)
      }
          ...
          <div className="container flex h-16 items-center">
                ...
                <div className="flex flex-col h-full">
                  <div className="p-6 border-b">
                    <Link href="/" className="flex items-center gap-2 mb-6">
                      <span className="font-bold text-xl gradient-text">STREAMFLIX</span>
                    </Link>
                  ...
                  <div className="flex-1 overflow-auto py-6 px-4">
                    <div className="space-y-6">
                      <div>
                        <h3 className="text-sm font-medium text-muted-foreground mb-3 px-2">Menu</h3>
                        <nav className="flex flex-col gap-1">
                          <Link
                            href="/"
                            className={`flex items-center gap-3 px-2 py-2 text-sm rounded-md ${
                              pathname === "/" ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground"
                            }`}
                          >
            ...
            <nav className="hidden md:flex items-center gap-6 text-sm">
              ...
              <Link
                href="/shows"
                className={`font-medium transition-colors hover:text-primary flex items-center gap-2 ${
                  pathname?.startsWith("/shows") ? "text-primary" : "text-foreground/60"
                }`}
              >
              ...
              <Link
                href="/movies"
                className={`font-medium transition-colors hover:text-primary flex items-center gap-2 ${
                  pathname?.startsWith("/movies") ? "text-primary" : "text-foreground/60"
                }`}
              >
              ...
              <Link
                href="/watchlist"
                className={`font-medium transition-colors hover:text-primary flex items-center gap-2 ${
                  pathname?.startsWith("/watchlist") ? "text-primary" : "text-foreground/60"
                }`}
              >
            ...
            <div className="ml-auto flex items-center gap-2">
              {showSearch ? (
                <form
                  ...
                  className="flex items-center bg-muted rounded-full overflow-hidden pl-3 transition-all"
                >
                  <Search className="h-4 w-4 text-muted-foreground" />
                  <Input
                    type="search"
                    placeholder="Search..."
                    className="w-[200px] md:w-[300px] border-0 bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                  />
              ...
              {user && (
                <div className="relative">
                  ...
                  {showNotifications && (
                    <div className="absolute right-0 mt-2 w-80 bg-popover rounded-md shadow-lg overflow-hidden z-50 border">
              ...
              {user ? (
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-56">
                    ...
                    <DropdownMenuItem onClick={() => router.push("/profile/subscriptions")}>
